...
def homepage():
	...
	if len(likes) == 0 or len(dislikes) == 0:
		...
		if (len(likes) + len(dislikes)) % 10 == 0 and (len(likes) + len(dislikes)) > 0 \
		  or ic.model is None:
			...
			#clustering.count_votes_by_cluster(likes, dislikes)
			#bb.trials = clustering.bb.trials
			#bb.successes = clustering.bb.successes
			#bb.num_options = clustering.bb.num_options
			#print 'num_options', bb.num_options
			#bb.prior = clustering.bb.prior
